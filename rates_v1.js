/** These rates are based off of the current and the recommended rate proposals  */

const sewerRates = {
    fixed: {
        2024: {
            insideStandardWastewater: 45.60, // 'Residential', 'Mobile Home Parks (Per Space)'
            motelsAndHotelsPerUnit: 7.59, // 'Motels', 'Hotels'
            motelsAndHotelsPerLivingUnit: 45.60, // 'Motels', 'Hotels'
            commercialLessThanOrEqualToOneInchMeter: 163.28, // 'Restaurants', 'Supermarkets', 'Mortuaries', 'Bakeries'
            commercialGreaterThanOneInchMeter: 266.34, // 'Restaurants', 'Supermarkets', 'Mortuaries', 'Bakeries'
            laundries: 24.20, // 'Laundries'
            insideCommercial: {
                '5/8"': 45.60, // City Park Restroom
                '3/4"': 72.02,
                '1"': 103.48,
                '1 1/2"': 186.06,
                '2"': 291.55,
                '3"': 537.41,
                '4"': 875.43,
                '6"': 1753.50,
                '8"': 2820.52,
            },
        },
        2025: 45.46,
        2026: 50.01,
        2027: 55.01,
        2028: 60.51,
        2029: 66.56,
    },
    variable: {
        2025: 5.50,
        2026: 6.05,
        2027: 6.66,
        2028: 7.32,
        2029: 8.05,
    },
}

const waterRates = {
    fixed: {
        domestic: {
            2024: {
                '5/8"': 27.09,
                '3/4"': 36.46,
                '1"': 55.18,
                '1 1/2"': 101.98,
                '2"': 158.13,
                '3"': 335.95,
                '4"': 598.02,
                '6"': 1505.89,
                '8"': 2629.02,
                '10"': 3939.33,
            },
            2025: {
                '5/8"': 32.79,
                '3/4"': 42.94,
                '1"': 63.23,
                '1 1/2"': 113.94,
                '2"': 174.81,
                '3"': 367.54,
                '4"': 651.56,
                '6"': 1635.51,
                '8"': 2852.76,
                '10"': 4272.88,
            },
            2026: {
                '5/8"': 35.75,
                '3/4"': 46.80,
                '1"': 68.92,
                '1 1/2"': 124.20,
                '2"': 190.54,
                '3"': 400.62,
                '4"': 710.20,
                '6"': 1782.70,
                '8"': 3109.51,
                '10"': 4657.44,
            },
            2027: {
                '5/8"': 38.96,
                '3/4"': 51.02,
                '1"': 75.12,
                '1 1/2"': 135.38,
                '2"': 207.69,
                '3"': 436.67,
                '4"': 774.12,
                '6"': 1943.15,
                '8"': 3389.36,
                '10"': 5076.61,
            },
            2028: {
                '5/8"': 42.47,
                '3/4"': 55.61,
                '1"': 81.88,
                '1 1/2"': 147.56,
                '2"': 226.38,
                '3"': 475.97,
                '4"': 843.79,
                '6"': 2118.03,
                '8"': 3694.40,
                '10"': 5533.51,
            },
            2029: {
                '5/8"': 46.29,
                '3/4"': 60.61,
                '1"': 89.25,
                '1 1/2"': 160.84,
                '2"': 246.75,
                '3"': 518.81,
                '4"': 919.74,
                '6"': 2308.65,
                '8"': 4026.90,
                '10"': 6031.52,
            },
        },
        privateFire: {
            2024: {
                '5/8"': 8.60,
                '3/4"': 8.60,
                '1"': 8.60,
                '1 1/2"': 8.60,
                '2"': 9.74,
                '3"': 10.81,
                '4"': 16.76,
                '6"': 32.74,
                '8"': 60.29,
                '10"': 101.72,
            },
            2025: {
                '5/8"': 12.64,
                '3/4"': 12.64,
                '1"': 12.64,
                '1 1/2"': 13.33,
                '2"': 14.91,
                '3"': 17.63,
                '4"': 27.38,
                '6"': 44.20,
                '8"': 69.50,
                '10"': 104.57,
            },
            2026: {
                '5/8"': 13.78,
                '3/4"': 13.78,
                '1"': 13.78,
                '1 1/2"': 14.53,
                '2"': 16.25,
                '3"': 19.21,
                '4"': 29.84,
                '6"': 48.18,
                '8"': 75.76,
                '10"': 113.98,
            },
            2027: {
                '5/8"': 15.02,
                '3/4"': 15.02,
                '1"': 15.02,
                '1 1/2"': 15.84,
                '2"': 17.71,
                '3"': 20.94,
                '4"': 32.53,
                '6"': 52.51,
                '8"': 82.57,
                '10"': 124.24,
            },
            2028: {
                '5/8"': 16.37,
                '3/4"': 16.37,
                '1"': 16.37,
                '1 1/2"': 17.27,
                '2"': 19.31,
                '3"': 22.83,
                '4"': 35.46,
                '6"': 57.24,
                '8"': 90.01,
                '10"': 135.42,
            },
            2029: {
                '5/8"': 17.84,
                '3/4"': 17.84,
                '1"': 17.84,
                '1 1/2"': 18.82,
                '2"': 21.05,
                '3"': 24.88,
                '4"': 38.65,
                '6"': 62.39,
                '8"': 98.11,
                '10"': 147.61,
            },
        },
    },
    variable: {
        residential: {
            2024: {
                tier1: 1.93,
                tier2: 2.77,
                tier3: 5.48,
                tier4: 9.12,
                tier5: 13.59,
            },
            2025: {
                tier1: 1.96,
                tier2: 3.31,
                tier3: 4.38,
                tier4: 5.22,
            },
            2026: {
                tier1: 2.13,
                tier2: 3.60,
                tier3: 4.77,
                tier4: 5.69,
            },
            2027: {
                tier1: 2.32,
                tier2: 3.93,
                tier3: 5.20,
                tier4: 6.20,
            },
            2028: {
                tier1: 2.53,
                tier2: 4.28,
                tier3: 5.67,
                tier4: 6.76,
            },
            2029: {
                tier1: 2.76,
                tier2: 4.67,
                tier3: 6.18,
                tier4: 7.36,
            },
        },
        commercial: {
            2024: {
                tier1: 2.77,
                tier2: 5.48,
                tier3: 9.12,
                tier4: 13.59,
            },
            2025: {
                tier1: 3.31,
                tier2: 3.48,
                tier3: 5.22,
            },
            2026: {
                tier1: 3.60,
                tier2: 4.77,
                tier3: 5.69,
            },
            2027: {
                tier1: 3.93,
                tier2: 5.20,
                tier3: 6.20,
            },
            2028: {
                tier1: 4.28,
                tier2: 5.67,
                tier3: 6.76,
            },
            2029: {
                tier1: 4.67,
                tier2: 6.18,
                tier3: 7.36,
            },
        },
    },
}

const reclaimedWaterRates = {
    fixed: {
        2024: {
            '5/8"': 23.52,
            '3/4"': 30.81,
            '1"': 45.39,
            '1 1/2"': 81.84,
            '2"': 125.59,
            '3"': 264.13,
            '4"': 468.28,
            '6"': 1175.53,
            '8"': 2050.48,
            '10"': 3071.25,
        },
        2025: {
            '5/8"': 26.64,
            '3/4"': 35.95,
            '1"': 54.57,
            '1 1/2"': 101.12,
            '2"': 156.98,
            '3"': 333.88,
            '4"': 594.58,
            '6"': 1497.70,
            '8"': 2614.96,
            '10"': 3918.44,
        },
        2026: {
            '5/8"': 27.70,
            '3/4"': 37.38,
            '1"': 56.75,
            '1 1/2"': 105.16,
            '2"': 163.26,
            '3"': 347.24,
            '4"': 618.36,
            '6"': 1557.61,
            '8"': 2719.56,
            '10"': 4075.17,
        },
        2027: {
            '5/8"': 28.81,
            '3/4"': 38.88,
            '1"': 59.02,
            '1 1/2"': 109.37,
            '2"': 169.79,
            '3"': 361.13,
            '4"': 643.10,
            '6"': 1619.91,
            '8"': 2828.34,
            '10"': 4238.18,
        },
        2028: {
            '5/8"': 29.96,
            '3/4"': 40.43,
            '1"': 61.38,
            '1 1/2"': 113.75,
            '2"': 176.58,
            '3"': 375.57,
            '4"': 668.82,
            '6"': 1684.71,
            '8"': 2941.48,
            '10"': 4407.71,
        },
        2029: {
            '5/8"': 31.16,
            '3/4"': 42.05,
            '1"': 63.84,
            '1 1/2"': 118.30,
            '2"': 183.65,
            '3"': 390.60,
            '4"': 695.57,
            '6"': 1752.10,
            '8"': 3059.14,
            '10"': 4584.02,
        },
    },
    variable: {
        2024: {
            tier1: 2.14,
            tier2: 3.21,
            tier3: 4.27,
            tier4: 6.41,
        },
        2025: {
            tier1: 2.37,
            tier2: 2.60,
        },
        2026: {
            tier1: 2.46,
            tier2: 2.71,
        },
        2027: {
            tier1: 2.56,
            tier2: 2.82,
        },
        2028: {
            tier1: 2.66,
            tier2: 2.93,
        },
        2029: {
            tier1: 2.77,
            tier2: 3.05,
        },
    },
}

const electricRates = {
    residential: {
        2024: {
            tier1: 0.11808,
            tier2: 0.13741,
            tier3: 0.22696,
            tier4: 0.32337,
            fixed: {
                fixedSingleFamily: 0.88,
                fixedMultiFamily: 0.67,
            },
        },
        2025: {
            tier1: 0.10100,
            tier2: 0.11100,
            tier3: 0.21000,
            tier4: 0.21000,
            publicBenefits: 0.00378,
            fixed: 15.90,
        },
        2026: {
            tier1: 0.10504,
            tier2: 0.11544,
            tier3: 0.21840,
            tier4: 0.21840,
            publicBenefits: 0.00405,
            fixed: 16.54,
        },
        2027: {
            tier1: 0.10924,
            tier2: 0.12006,
            tier3: 0.22714,
            tier4: 0.22714,
            publicBenefits: 0.00419,
            fixed: 17.20,
        },
        2028: {
            tier1: 0.11361,
            tier2: 0.12486,
            tier3: 0.23623,
            tier4: 0.23623,
            publicBenefits: 0.00433,
            fixed: 17.89,
        },
        2029: {
            tier1: 0.11815,
            tier2: 0.12985,
            tier3: 0.24568,
            tier4: 0.24568,
            publicBenefits: 0.00448,
            fixed: 18.61,
        },
    },
    smallCommercialGS1: {
        2024: {
            fixed: {
                singlePhase: 12.99,
                threePhase: 3.16,
            },
            variable: {
                summer: 0.172800,
                winter: 0.168720,
            }
        },
        2025: {
            publicBenefits: 0.00378,
            fixed: {
                singlePhase: 20.97,
                threePhase: 3.16,
            },
            variable: 0.15500,
        },
        2026: {
            publicBenefits: 0.00405,
            fixed: {
                singlePhase: 21.81,
                threePhase: 3.29,
            },
            variable: 0.16120,
        },
        2027: {
            publicBenefits: 0.00419,
            fixed: {
                singlePhase: 22.68,
                threePhase: 3.42,
            },
            variable: 0.16765,
        },
        2028: {
            publicBenefits: 0.00433,
            fixed: {
                singlePhase: 23.59,
                threePhase: 3.56,
            },
            variable: 0.17436,
        },
        2029: {
            publicBenefits: 0.00448,
            fixed: {
                singlePhase: 24.53,
                threePhase: 3.70,
            },
            variable: 0.18133,
        },
    },
    mediumCommercialGS2: {
        2024: {
            fixed: 71.50,
            kwDemandPowerSupplyPeak: 20.67,
            kwDemandDistributionFacilities: 7.35,
            kWhEnergy: {
                summer: 0.096480,
                winter: 0.087380,
            },
        },
        2025: {
            fixed: 35.13,
            kwDemandPowerSupplyPeak: 8.27,
            kwDemandDistributionFacilities: 15.74,
            kWhEnergy: 0.05905,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 36.54,
            kwDemandPowerSupplyPeak: 8.60,
            kwDemandDistributionFacilities: 16.37,
            kWhEnergy: 0.06141,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 38.00,
            kwDemandPowerSupplyPeak: 8.94,
            kwDemandDistributionFacilities: 17.02,
            kWhEnergy: 0.06387,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 39.52,
            kwDemandPowerSupplyPeak: 9.30,
            kwDemandDistributionFacilities: 17.70,
            kWhEnergy: 0.06642,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 41.10,
            kwDemandPowerSupplyPeak: 9.67,
            kwDemandDistributionFacilities: 18.41,
            kWhEnergy: 0.06908,
            publicBenefits: 0.00448,
        },
    },
    largeCommercialTOUGS3: {
        2024: {
            fixed: 277.25,
            kWDemandPowerSupplyPeak: {
                summerPeak: 18.16,
                summerMidPeak: 6.23,
            },
            kWDemandDistributionFacilities: 7.62,
            kWhEnergy: {
                summerOnPeak: 0.13561,
                summerMidPeak: 0.11027,
                SummerOffPeak: 0.07706,
                WinterMidPeak: 0.11282,
                WinterOffPeak: 0.08052,
            },
            kVaPowerFactorAdjustment: 0.18000,
        },
        2025: {
            fixed: 49.30,
            kWDemandPowerSupplyPeak: 8.29,
            kWDemandDistributionFacilities: 17.51,
            kWhEnergy: {
                summerOnPeak: 0.09620,
                summerMidPeak: 0.07280,
                SummerOffPeak: 0.05200,
                WinterMidPeak: 0.07986,
                WinterOffPeak: 0.05200,
            },
            kVaPowerFactorAdjustment: 0.18000,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 51.27,
            kWDemandPowerSupplyPeak: 8.62,
            kWDemandDistributionFacilities: 18.21,
            kWhEnergy: {
                summerOnPeak: 0.10005,
                summerMidPeak: 0.07571,
                SummerOffPeak: 0.05408,
                WinterMidPeak: 0.08305,
                WinterOffPeak: 0.05408,
            },
            kVaPowerFactorAdjustment: 0.18720,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 53.32,
            kWDemandPowerSupplyPeak: 8.96,
            kWDemandDistributionFacilities: 18.94,
            kWhEnergy: {
                summerOnPeak: 0.10405,
                summerMidPeak: 0.07874,
                SummerOffPeak: 0.05624,
                WinterMidPeak: 0.08637,
                WinterOffPeak: 0.05624,
            },
            kVaPowerFactorAdjustment: 0.19469,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 55.45,
            kWDemandPowerSupplyPeak: 9.32,
            kWDemandDistributionFacilities: 19.70,
            kWhEnergy: {
                summerOnPeak: 0.10821,
                summerMidPeak: 0.08189,
                SummerOffPeak: 0.05849,
                WinterMidPeak: 0.08982,
                WinterOffPeak: 0.05849,
            },
            kVaPowerFactorAdjustment: 0.20248,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 57.67,
            kWDemandPowerSupplyPeak: 9.69,
            kWDemandDistributionFacilities: 20.49,
            kWhEnergy: {
                summerOnPeak: 0.11254,
                summerMidPeak: 0.08517,
                SummerOffPeak: 0.06083,
                WinterMidPeak: 0.09341,
                WinterOffPeak: 0.06083,
            },
            kVaPowerFactorAdjustment: 0.21058,
            publicBenefits: 0.00448,
        },
    },
    industrialTOU8: {
        2024: {
            fixed: 346.00,
            kWDemandPowerSupplyPeak: {
                summerPeak: 16.91,
                summerMidPeak: 5.71,
            },
            kWDemandDistributionFacilities: 8.31,
            kWhEnergy: {
                summerOnPeak: 0.12675,
                summerMidPeak: 0.10299,
                summerOffPeak: 0.07184,
                winterMidPeak: 0.10538,
                winterOffPeak: 0.07509,
            },
            kVaPowerFactorAdjustment: 0.18000,
        },
        2025: {
            fixed: 76.97,
            kWDemandPowerSupplyPeak: 8.31,
            kWDemandDistributionFacilities: 15.06,
            kWhEnergy: {
                summerOnPeak: 0.09630,
                summerMidPeak: 0.07319,
                summerOffPeak: 0.05200,
                winterMidPeak: 0.07986,
                winterOffPeak: 0.05200,
            },
            kVaPowerFactorAdjustment: 0.18000,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 80.05,
            kWDemandPowerSupplyPeak: 8.64,
            kWDemandDistributionFacilities: 15.66,
            kWhEnergy: {
                summerOnPeak: 0.10015,
                summerMidPeak: 0.07611,
                summerOffPeak: 0.05408,
                winterMidPeak: 0.08305,
                winterOffPeak: 0.05408,
            },
            kVaPowerFactorAdjustment: 0.18720,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 83.25,
            kWDemandPowerSupplyPeak: 8.99,
            kWDemandDistributionFacilities: 16.29,
            kWhEnergy: {
                summerOnPeak: 0.10416,
                summerMidPeak: 0.07915,
                summerOffPeak: 0.05624,
                winterMidPeak: 0.08637,
                winterOffPeak: 0.05624,
            },
            kVaPowerFactorAdjustment: 0.19469,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 86.58,
            kWDemandPowerSupplyPeak: 9.35,
            kWDemandDistributionFacilities: 16.94,
            kWhEnergy: {
                summerOnPeak: 0.10833,
                summerMidPeak: 0.08232,
                summerOffPeak: 0.05849,
                winterMidPeak: 0.08982,
                winterOffPeak: 0.05849,
            },
            kVaPowerFactorAdjustment: 0.20248,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 90.04,
            kWDemandPowerSupplyPeak: 9.72,
            kWDemandDistributionFacilities: 17.62,
            kWhEnergy: {
                summerOnPeak: 0.11266,
                summerMidPeak: 0.08561,
                summerOffPeak: 0.06083,
                winterMidPeak: 0.09341,
                winterOffPeak: 0.06083,
            },
            kVaPowerFactorAdjustment: 0.21058,
            publicBenefits: 0.00448,
        },
    },
    outdoorAreaLightingAL2: {
        2024: {
            fixed: 7.00,
            kWhEnergy: 0.08224,
        },
        2025: {
            fixed: 10.26,
            kWhEnergy: 0.11678,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 10.67,
            kWhEnergy: 0.12145,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 11.10,
            kWhEnergy: 0.12631,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 11.54,
            kWhEnergy: 0.13136,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 12.01,
            kWhEnergy: 0.13661,
            publicBenefits: 0.00448,
        },
    },
    streetLightingLS3: {
        2024: {
            fixed: 7.00,
            kWhEnergy: 0.08224,
        },
        2025: {
            fixed: 10.26,
            kWhEnergy: 0.11678,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 10.67,
            kWhEnergy: 0.12145,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 11.10,
            kWhEnergy: 0.12631,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 11.54,
            kWhEnergy: 0.13136,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 12.01,
            kWhEnergy: 0.13661,
            publicBenefits: 0.00448,
        },
    },
    trafficControlTC1: {
        2024: {
            fixed: {
                singlePhase: 9.49,
                threePhase: 3.16,
            },
            kWhEnergy: 0.11407,
        },
        2025: {
            fixed: {
                singlePhase: 13.91,
                threePhase: 4.64,
            },
            kWhEnergy: 0.16345,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: {
                singlePhase: 14.47,
                threePhase: 4.82,
            },
            kWhEnergy: 0.17598,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: {
                singlePhase: 15.05,
                threePhase: 5.02,
            },
            kWhEnergy: 0.18994,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: {
                singlePhase: 15.65,
                threePhase: 5.22,
            },
            kWhEnergy: 0.20852,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: {
                singlePhase: 16.28,
                threePhase: 5.43,
            },
            kWhEnergy: 0.23192,
            publicBenefits: 0.00448,
        },
    },
    commercialEVChargingRate: {
        2025: {
            fixed: 76.97,
            kWDemand: 15.06,
            kWhEnergyOnPeak4to9PM: 0.19710,
            kWhEnergyOffPeakAllOtherHours: 0.06570,
            kVaPowerFactorAdjustment: 0.18000,
            publicBenefits: 0.00378,
        },
        2026: {
            fixed: 80.05,
            kWDemand: 15.66,
            kWhEnergyOnPeak4to9PM: 0.20498,
            kWhEnergyOffPeakAllOtherHours: 0.06833,
            kVaPowerFactorAdjustment: 0.18720,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 83.25,
            kWDemand: 16.29,
            kWhEnergyOnPeak4to9PM: 0.21318,
            kWhEnergyOffPeakAllOtherHours: 0.07106,
            kVaPowerFactorAdjustment: 0.19469,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 86.58,
            kWDemand: 16.94,
            kWhEnergyOnPeak4to9PM: 0.22171,
            kWhEnergyOffPeakAllOtherHours: 0.07390,
            kVaPowerFactorAdjustment: 0.20248,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 90.04,
            kWDemand: 17.62,
            kWhEnergyOnPeak4to9PM: 0.23058,
            kWhEnergyOffPeakAllOtherHours: 0.07686,
            kVaPowerFactorAdjustment: 0.21058,
            publicBenefits: 0.00448,
        },
    },
    pumpingAndAgriculture: {
        2024: {
            fixed: 63.25,
            kWDemandPowerSupplyPeak: 10.85,
            kWDemandDistributionFacilities: 3.85,
            kWhEnergy: {
                summerOnPeak: 0.13064,
                summerMidPeak: 0.10929,
                summerOffPeak: 0.05226,
                winterMidPeak: 0.12255,
                winterOffPeak: 0.05226,
            },
            kVaPowerFactorAdjustment: 0.18000,
        },
        2025: {
            january: {
                fixed: 19.14,
                kWDemandPowerSupplyPeak: 6.00,
                kWDemandDistributionFacilities: 8.00,
                kWhEnergy: {
                    summerOnPeak: 0.10361,
                    summerMidPeak: 0.07874,
                    summerOffPeak: 0.05595,
                    winterMidPeak: 0.08592,
                    winterOffPeak: 0.05595,
                },
                kVaPowerFactorAdjustment: 0.18000,
                publicBenefits: 0.00378,
            },
            july: {
                fixed: 19.14,
                kWDemandPowerSupplyPeak: 12.00,
                kWDemandDistributionFacilities: 10.00,
                kWhEnergy: {
                    summerOnPeak: 0.10361,
                    summerMidPeak: 0.07874,
                    summerOffPeak: 0.05595,
                    winterMidPeak: 0.08592,
                    winterOffPeak: 0.05595,
                },
                kVaPowerFactorAdjustment: 0.18000,
                publicBenefits: 0.00378,
            },
        },
        2026: {
            fixed: 20.86,
            kWDemandPowerSupplyPeak: 12.98,
            kWDemandDistributionFacilities: 21.55,
            kWhEnergy: {
                summerOnPeak: 0.09444,
                summerMidPeak: 0.07178,
                summerOffPeak: 0.05100,
                winterMidPeak: 0.07832,
                winterOffPeak: 0.05100,
            },
            kVaPowerFactorAdjustment: 0.18720,
            publicBenefits: 0.00405,
        },
        2027: {
            fixed: 21.69,
            kWDemandPowerSupplyPeak: 13.50,
            kWDemandDistributionFacilities: 22.41,
            kWhEnergy: {
                summerOnPeak: 0.09822,
                summerMidPeak: 0.07465,
                summerOffPeak: 0.05304,
                winterMidPeak: 0.08145,
                winterOffPeak: 0.05304,
            },
            kVaPowerFactorAdjustment: 0.19469,
            publicBenefits: 0.00419,
        },
        2028: {
            fixed: 22.56,
            kWDemandPowerSupplyPeak: 14.04,
            kWDemandDistributionFacilities: 23.31,
            kWhEnergy: {
                summerOnPeak: 0.10215,
                summerMidPeak: 0.07764,
                summerOffPeak: 0.05516,
                winterMidPeak: 0.08471,
                winterOffPeak: 0.05516,
            },
            kVaPowerFactorAdjustment: 0.20248,
            publicBenefits: 0.00433,
        },
        2029: {
            fixed: 23.46,
            kWDemandPowerSupplyPeak: 14.60,
            kWDemandDistributionFacilities: 24.24,
            kWhEnergy: {
                summerOnPeak: 0.10624,
                summerMidPeak: 0.08075,
                summerOffPeak: 0.05737,
                winterMidPeak: 0.08810,
                winterOffPeak: 0.05737,
            },
            kVaPowerFactorAdjustment: 0.21058,
            publicBenefits: 0.00448,
        }
    }
}